...
export const genTemplateBlock = function (html, file) {
...
export const genImportBlock = function (statements, file) {
  const layout = normalizeLayout(file.data.layout)
  let code = statements.concat(`import VueRemarkRoot from ${JSON.stringify(layout.component)}`).join('\n')
...
export const genFrontMatterBlock = function (data) {
