...
import createHTMLRenderer from '../server/createHTMLRenderer.js'
import CSSExtractPlugin from 'mini-css-extract-plugin'
import corejsresolver from './plugins/CoreJSResolver.js'
...
export default (app, { isProd, isServer }) => {
      ...
      rule.use('postcss-loader')
        ...
        .options({
          ...
          postcssOptions: !postcssConfigFiles.length
            ? {
              ...postcssOptions,
              plugins: [...(postcssOptions.plugins || []), autoprefixer]
            }
            : {}
        })
  ...
  if (projectConfig.cache) {
    config.merge({
      cache: {
        type: 'filesystem',
        version: hash(app.compiler.hooks.cacheIdentifier.call({
          'gridsome': package$0.version,
          'vue-loader': package$1.version,
          'context': app.context,
          'env': gridsomeEnv()
        })),
        buildDependencies: {
