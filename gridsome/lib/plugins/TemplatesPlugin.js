...
import { validateTypeName } from '../graphql/utils.js'
import { SUPPORTED_DATE_FORMATS } from '../utils/constants.js'
import { isPlainObject, trimStart, trimEnd, get, memoize } from 'lodash'
...
const makePath = (object, { routeKeys, createPath }, dateField = 'date', slugify) => {
  ...
  for (let i = 0; i < length; i++) {
    ...
    if (fieldName === 'id')
      params.id = object.id
    else if (fieldName === 'year' && !object.year)
      params.year = date().format('YYYY')
    else if (fieldName === 'month' && !object.month)
      params.month = date().format('MM')
    else if (fieldName === 'day' && !object.day)
      params.day = date().format('DD')
    else {
    ...
    api.createSchema(({ addSchemaResolvers }) => {
      ...
      for (const typeName of templates.byTypeName.keys()) {
        if (!typeNames.includes(typeName)) {
          ...
          throw new Error(`A content type for the ${typeName} template does not exist.` +
        ...
        for (const [index, tmpl] of typeTemplates.entries()) {
          if (tmpl.from === FROM_CONTENT_TYPE &&
...
class Template {
  ...
  remove() {
    if (this.route)
      this.actions.removeRoute(this.route.id)
    else
      this.actions.removePagesByComponent(this.component)
  }
  createPage(node) {
    ...
    const path = node.internal.publicPath
      ...
      : node.path
    if (this.route)
      this.route.addPage({ id, path, queryVariables })
    else
      this.actions.createPage({ id, path, component, queryVariables })
  }
  updatePage(node, oldNode) {
    const { path, name } = this

    if (node.internal.publicPath[name] !== oldNode.internal.publicPath[name] &&
            typeof path !== 'string') {
      this.actions.removePage(makeId(node.$uid, this.name))
    }
