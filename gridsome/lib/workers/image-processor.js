...
export const processImage = async function ({ width, height, filePath, destPath, imagesConfig, options = {} }) {
...
export const process = async function ({ queue, context, imagesConfig }) {
  await pMap(queue, async (set) => {
    try {
      await processImage({ ...set, imagesConfig })
    }
    catch (err) {
