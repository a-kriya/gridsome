...
class Store {
  constructor(app) {
    ...
    this.hooks.addNode.tap('TransformNodeContent', transformNodeContent)
    this.hooks.addNode.tap('ProcessNodeFields', processNodeFields)
  }
  getHooksContext() {
    return {
      hooks: this.hooks
  ...
  addCollection(options, store) {
    ...
    const collection = new Collection(options.typeName, options, store)
