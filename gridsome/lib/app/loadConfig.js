...
const builtInPlugins = [
  path.resolve(getDirname(), '../plugins/vue-components'),
  path.resolve(getDirname(), '../plugins/vue-pages'),
  path.resolve(getDirname(), '../plugins/RedirectsPlugin.js'),
  path.resolve(getDirname(), '../plugins/TemplatesPlugin.js')
]
...
function resolvePkg(context) {
  ...
  if (!dependencies.includes('@kriya/gridsome') &&
...
function normalizeTemplates(context, config, localConfig) {
  ...
  const normalize = (typeName, options, i = 0) => {
    ...
    if (Array.isArray(res[typeName]) &&
            res[typeName].find(tpl => tpl.name === options.name)) {
      throw new Error(`A template for "${typeName}" with the name "${options.name}" already exist.`)
    }
...
function normalizeRedirects(config) {
  ...
  if (Array.isArray(config.redirects)) {
    return config.redirects.map(rule => {
      const { error, value } = redirectSchema.validate(rule)
...
const permalinksSchema = Joi.object()
  ...
  .keys({
    ...
    slugify: Joi.alternatives()
      .try(
        Joi.object().keys({
          use: Joi.alternatives().try(Joi.string(), Joi.func()),
          options: Joi.object()
        }),
        Joi.func()
      )
...
function normalizePermalinks(permalinks = {}) {
  ...
  if (value.slugify && typeof value.slugify.use === 'string') {
    value.slugify.use = importSync(value.slugify.use)
  }
...
function resolveTransformers(pkg, config) {
  ...
  for (let id of deps) {
    ...
    const TransformerClass = importSync(id)
...
const imageSchema = Joi.object().label('Images').keys({
  ...
  defaultBlur: Joi.number().default(defaultPlaceholderImage.defaultBlur),
  ...
  placeholder: Joi.alternatives()
    ...
    .try(
      Joi.object().label('Blur').keys({
        ...
        defaultBlur: Joi.number().min(0).default(defaultPlaceholderImage.defaultBlur)
      }),
      Joi.object().label('Trace').keys({
        type: Joi.string().required().valid('trace'),
        background: Joi.string().empty('').default(() => undefined),
        color: Joi.string().empty('').default(() => undefined),
...
function normalizeImages(config = {}) {
  ...
  const { error, value } = imageSchema.validate(config)
...
export default async (context, options = {}) => {
  ...
  if ([configEntryPath, serverEntryPath].filter(isTS).length) {
    console.log(chalk.yellow('warn'), '- TypeScript support for the config and server entries is experimental and may introduce breaking changes at any time.\n')
    registerTsExtension()
  }
  ...
  if (options.useBuiltIn !== false) {
    ...
    plugins.unshift({
      use: path.resolve(getDirname(), '../plugins/core'),
      options: config
    })
  }
  ...
  config.appPath = path.resolve(getDirname(), '../../app')
