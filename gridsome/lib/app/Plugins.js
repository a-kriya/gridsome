...
class Plugins {
  constructor(app) {
    ...
    app.hooks.bootstrap.tapPromise({ name: 'loadSource', label: 'Load sources', phase: BOOTSTRAP_SOURCES }, () => this.loadSources())
    app.hooks.bootstrap.tapPromise({ name: 'createSchema', label: 'Create GraphQL schema', phase: BOOTSTRAP_GRAPHQL }, () => this.createSchema())
    app.hooks.bootstrap.tapPromise({ name: 'createPages', label: 'Create pages and templates', phase: BOOTSTRAP_PAGES }, () => this.createPages())
  }
  initialize() {
  ...
  async createSchema() {
    ...
    results.forEach(schema => schema && this._app.schema._schemas.push(schema))
  ...
  async run(eventName, cb, ...args) {
    ...
    for (const entry of this._listeners[eventName]) {
      if (entry.options.once && entry.done)
        continue
